...
def cloud_turn_on():
    try:
        r = requests.post(f"https://{BULB_IP}:5000/on", verify=False)
...
def cloud_turn_off():
    try:
        r = requests.post(f"https://{BULB_IP}:5000/off", verify=False)
...
def cloud_brightness():
    ...
    try:
        r = requests.post(f"https://{BULB_IP}:5000/brightness", json={"level": level}, verify=False)
...
def cloud_status():
    try:
        r = requests.get(f"https://{BULB_IP}:5000/status", verify=False)
        return r.json(), 200
    except Exception as e:
        return {"error": str(e)}, 500

if __name__ == "__main__":
    app.run(host="172.18.106.184", port=6000, ssl_context=("cert.pem", "key.pem"))
